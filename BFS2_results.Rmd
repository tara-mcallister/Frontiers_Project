...
round(c(mean_session_to_session_change_3, mean_session_to_session_change_4, mean_session_to_session_change_5), digits=2) #1 answer Adrian & Liam stand out
...
for (i in Children){
  i <- subset(BL_sessions, Subject == i)
  rising <- rbind(rising, subset(i, i$diff > i$sd))
}
rising   #3 answer:  no participant showed last 3 session diff greater than sd
...
For convenience, the single-subject graphs have been grouped into three sets of 3-4 participants who demonstrated broadly similar patterns of response to treatment. Figure 2 depicts four participants for whom visual inspection provided strong evidence of a response to at least one type of treatment, and Figure 3 shows three participants for whom visual inspection provided moderate support of a response to treatment. Finally, Figure 3 shows four participants who showed no reliable evidence of a response to treatment. The participants in each group are discussed in detail below. **Comment on baseline stability: Which participants met the criterion of <10% mean session-to-session variability over the baseline interval? Which participants have demonstrations of noneffect in the baseline phase?** All participants met the criterion of <10% mean session-to-session variability, with the highest variability from Adrian (`r round(mean_session_to_session_change_3[1], digits=2)`) and Liam (`r round(mean_session_to_session_change_4[4], digits=2)`). There were no outlier baseline sessions that exceeded 2 standard deviations from the mean BL percentage for each participant. Finally, no particpant showed a rising trend in baseline sessions, as the percentage differences between the last 3 baseline sessions for each participant did not exceed the standard deviation for that participant.
