...
In the logistic mixed model described above, neither treatment condition (TRAD versus BF) nor order of treatment delivery was a significant predictor of /r/ production accuracy. However, the interaction between treatment type and treatment order was significant (*$\beta$* = `r #COEF`, *SE* = `r #COEF`, , *p* = `r #COEF`). This interaction can be visualized in Figure 5, which plots accuracy in within-treatment probes (pre- and post-probes) during both biofeedback and traditional treatment conditions; they are pooled across subjects but partitioned by the order of treatment application (BF-first or TRAD-first). Figure 5 also shows that accuracy scores tended to be higher in the BF-first order than the TRAD-first order, although this effect was not significant (*$\beta$* = `r #COEF`, *SE* = `r #COEF`, , *p* = `r #COEF`). Complete regression results are reported in On-line Supplement A. FIX: Should use phase designation (phase 1 vs phase 2) instead phase order (BF_TRAD vs TRAD_BF)?? ANSWER: No, that model does not converge with random slopes included; this model is a better alternative.
