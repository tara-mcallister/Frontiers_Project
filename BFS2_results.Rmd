...
c(mean_session_to_session_change_3, mean_session_to_session_change_4, mean_session_to_session_change_5) #1 answer Adrian & Liam stand out
...
-Include t-test? (or leave hypothesis-testing to mixed model?)
...
In the logistic mixed model described above, treatment condition (TRAD versus BF) was a significant predictor of /r/ production accuracy ((*$\beta$* = `r #pre_b`, *SE* = `r #pre_sd`, , *p* = `r #pre_p`), with the coefficient indicating that the reference level, traditional treatment, was associated with lower accuracy than post-treatment probes. Order of treatment delivery (biofeedback-first versus traditional-first) was not a significant predictor (*$\beta$* = `r #condition_b`, *SE* = `r #condition_sd`, , *p* = `r #condition_p`), but the interaction between treatment type and treatment order was significant (*$\beta$* = `r #int_b`, *SE* = `r #int_sd`, , *p* = `r #int_p`). This interaction can be visualized in Figure 5, which plots accuracy in within-treatment probes (pre- and post-probes) during both biofeedback and traditional treatment conditions; they are pooled across subjects but partitioned by the order of treatment application (BF-first or TRAD-first). Figure 5 shows that accuracy scores tended to be higher in the BF-first order than the TRAD-first order, although this effect was not significant. Furthermore, the main effect of condition (TRAD > BF) appears to be driven heavily by the BF-first group. Complete regression results are reported in On-line Supplement A. FIX: Should use phase designation (phase 1 vs phase 2) instead phase order (BF_TRAD vs TRAD_BF)??
